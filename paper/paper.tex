...
\urlstyle{same}
\usepackage[textsize=tiny,textwidth=2cm,shadow]{todonotes}
	
\newcommand{\skcom}[1]{\todo[color=orange!50!white]{{S}: #1}}
\newcommand{\rcom}[1]{\todo[color=gray!50!white]{{R}: #1}}%reviewers said
\usepackage{cleveref}
...
individual vertex and its neighbourhood \cite{DBLP:journals/csr/FialaK08}.\skcom{It seems a little bit strange to start with a definition. How about we describe it more wordy in the introduction? Or maybe I would start motivating with subgraph isomorphism(which has an easy definition i words) and the fact that this doesnt ideally match many causes... and then go to the definition of general homomorphism? } Finding any of these three kinds of homomorphism between
two given graphs is \NP-complete\skcom{cite a proof of this}.  The subgraph isomorphism problem arises in many practical
...
based upon finding a maximum clique in the neighbourhood of each pattern vertex, a
technique, that is valid even in the general case.\skcom{I wouldnt emphasize so much that "we prove" - its a common result and also pretty immediate..} Although this new filtering technique involves solving
...
path of length two (see \cref{figure:counterexamples}, top right). While standard injectivity prevents any kind of merging, local injectivity prevents two vertices from being merged, only if they are both in the neighbourhood of the same vertex.\skcom{added this}
So the second property does not hold in full generality, because, for example, a path of length three
...
\begin{proposition}[paths of length two are preserved by locally injective
    ...
    $i(w)$.
\end{proposition}\skcom{I am unsure whether not to write a general statement combining this and proposition 2.2.. but maybe that}
...
\end{corollary}\skcom{Do you think we should say it or just state in the definition sections that both of these '
\textit{are} homomorphisms}
...
a member of $\omega_1$, we initialise the clique solver with a lower bound of $n$\skcom{why n?} rather than
starting from scratch. We then find the maximum clique size for a third pattern vertex $p_3$, using
whichever lower bound of $|\omega_1|$ and $|\omega_2|$ is better (if both are valid), and so on.\skcom{I think after you state the first example we can just say that in general, For vertex v, we take the largest lowerbound of maxclique values in its neighbourhood (initialise to 0)?}\skcom{Maybe explicitly say in which order we take the vertices?}
...
the largest pattern vertex has a neighbourhood clique size of only 5 then this is irrelevant.\skcom{I would change this bit. I think it should be clear that on the second graph we \textit{dont} find a maximum clique, but what we want (for our cliques are preserved constraint) is cliques$\geq$ max size in the pattern vertex or in general of at least the sizes we find in the pattern vertex}

...
$\neighbourhood_G[v]$, is the neighbourhood plus $v$. The \emph{degree} of a vertex, $\degree_G(v)$,
...
for finding homomorphisms relies upon exploiting various invariants. For example, every recent
constraint programming solver makes use of the following result.

\begin{proposition}\label{proposition:degreends}
    Suppose a subgraph isomorphism $i$ exists from a graph $G$ to a graph $H$. Let
    $v$ be any vertex. Then:
    \begin{enumerate}
        \item The function cannot map $v$ to a vertex of lower degree, $\degree(v) \le \degree(i(v))$.
...
These invariants do \emph{not} hold for homomorphisms in general. As a simple counter-example, a
homomorphism may map a star graph (one central vertex of high degree, which is adjacent to many
other vertices, each of which has no other neighbours) onto a single edge. However, it is
straightforward to verify the following.

\begin{proposition}Both invariants in \cref{proposition:degreends} are also valid for locally
injective homomorphisms.\end{proposition}
...
\begin{proposition}\label{proposition:clique}
    Suppose we are finding a graph homomorphism $i$ from a graph $G$ to a graph $H$ where $H$ does
    not contain any loops. Suppose there is a $k$ vertex clique in $G$. Then the image of this
    clique under $i$ must also be a $k$ vertex clique.
\end{proposition}
